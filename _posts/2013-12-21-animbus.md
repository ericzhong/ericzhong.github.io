...
* `portal/utils/dispatch/core.py`
        ...
        class Dispatcher(object):
            ...
            def sendRequest(self):
                ...
                self.reqMgmt.addActionRecord(self.request, reqId)
    ...
    在`actionProxy`中定义的变量：

        KEY_ACTION = 'action'
        KEY_BODY = 'body'

    先忽略init部分那一堆未知的类；  
    从验证代码`_checkRequest()`看出：

      * 对于`sendRequest()`：`request`是个字典，必须包含key `action`和`body`
      * 对于`getResponse()`：`request`是个字典，必须包含key `requestId`

    `sendRequest()`先检查参数`request`格式合法性，然后产生一个"ID"，将当前任务交给`eventlet`后台异步处理，以后可用ID找回该任务；
